...
int igraph_indheap_init_array     (igraph_indheap_t *h, igraph_real_t* data, long int len) {
  ...
  memmove(h->stor_begin, data, (size_t) len*sizeof(igraph_real_t));
  for (i=0; i<len; i++) {
    h->index_begin[i]=i+1;
  }
...
int igraph_indheap_reserve        (igraph_indheap_t* h, long int size) {
  ...
  memmove(tmp1, h->stor_begin, (size_t) actual_size*sizeof(igraph_real_t));
  memmove(tmp2, h->index_begin, (size_t) actual_size*sizeof(long int));
  igraph_Free(h->stor_begin);
  igraph_Free(h->index_begin);
...
int igraph_d_indheap_reserve        (igraph_d_indheap_t* h, long int size) {
  ...
  memmove(tmp1, h->stor_begin, (size_t) actual_size*sizeof(igraph_real_t));
  memmove(tmp2, h->index_begin, (size_t) actual_size*sizeof(long int));
  memmove(tmp3, h->index2_begin, (size_t) actual_size*sizeof(long int));
  igraph_Free(h->stor_begin);
  igraph_Free(h->index_begin);
  igraph_Free(h->index2_begin);
